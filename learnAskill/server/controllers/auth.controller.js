...
exports.recruitersignup = async (req, res, next) => {
  try {
    ...
    if (!validator.isEmail(email) || !validator.isMobilePhone(mobile, "en-IN"))
      throw new Error("enter a valid email and valid phone number");

    let duplicateemail = await Recruiter.findOne({ email: email });
    let phonenumber = await Recruiter.findOne({ mobile: mobile });

    if (duplicateemail || phonenumber)
      throw new Error("please enter unique email and phone number");
...
exports.recruitersignin = async (req, res, next) => {
  try {
    ...
    if (!usersearchbyemail && !usersearchbymobile)
      throw new Error("enter valid email password");
