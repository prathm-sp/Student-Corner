...
function Dashboards() {
  ...
  useEffect(() => {
    ...
    axios.get('/class/student/list', { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
        console.log(res)

        setDate(res.data.application)
      })
  ...
  const handleClick = (val, id) => {
    console.log(val, id)
    if (val == "Accept") {
      axios.get(`/class/accepted/${id}`, { headers: { Authorization: `Bearer ${token}` } })
        .then((res) => {
          console.log(res);
      ...
      axios.get(`/class/rejected/${id}`, { headers: { Authorization: `Bearer ${token}` } })
        .then((res) => {
          console.log(res);
                                ...
                                <tbody>
                                  {data?.map?.((item) => (
                                    <tr className="unread">
                                      ...
                                      <td>
                                        <h4 className="mb-1">{item?.applicantid?.email}</h4>

                                      </td>
                                      <td>
                                        <h6 className="text-muted"><i className="fas fa-circle text-c-green f-10 m-r-15" />{item?.applicantid?.mobile}</h6>
                                      </td>
                                      <td>
                                        <h4 className="mb-1"><i className="fas fa-circle text-c-green f-10 m-r-15" />{item?.classid?.classname}</h4>

                                      </td>
                                      <td> <input onClick={() => { handleClick('Accept', item?._id) }} type="submit" value="Accpet" className="btn re btn-primary " style={{ width: "20vh", height: "6vh", marginLeft: "25%", marginTop: "0px", fontSize: "15px", backgroundColor: "#8eb553", borderRadius: "12px" }} defaultValue="" />
                                        <input onClick={() => { handleClick('Reject', item?._id) }} type="submit" value="Reject" className="btn re btn-primary " style={{ width: "20vh", marginLeft: "9%", height: "6vh", marginTop: "0px", fontSize: "15px", backgroundColor: "#da2461", borderRadius: "12px" }} defaultValue="" />
                                      </td>
                                    </tr>
                                  ))}
